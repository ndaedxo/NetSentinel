#!/usr/bin/env python3
"""
Threat Intelligence Integration for OpenCanary
...
class ThreatIndicator:
    ...
    def indicator_hash(self) -> str:
        ...
        return hashlib.sha256(f"{self.indicator_type}:{self.indicator}".encode()).hexdigest()
...
class ThreatFeed:
    ...
    def __init__(self, name: str, url: str, feed_type: str = "json",
...
class ThreatIntelligenceManager:
    ...
    def __init__(self, cache_file: str = "/tmp/opencanary_threat_intel.json",
        ...
        logger.info(f"Initialized threat intelligence manager with {len(self.indicators)} cached indicators")
        ...
        for feed_name, feed in self.feeds.items():
            ...
            try:
                ...
                if indicators:
                    ...
                    feed.mark_success()
                    logger.info(f"Successfully updated feed {feed_name}: {len(indicators)} indicators")
                else:
        ...
        try:
            ...
            for item in items:
                if isinstance(item, dict):
                    ...
                    indicator = item.get('ipAddress') or item.get('domain') or item.get('url') or item.get('hash')
                    if not indicator:
                        continue
    ...
    def _process_indicators(self, indicators: List[ThreatIndicator], feed: ThreatFeed):
        ...
        for indicator in indicators:
            ...
            if indicator_hash in self.indicators:
                ...
                if indicator.indicator_type in self.indicators_by_type:
                    self.indicators_by_type[indicator.indicator_type][indicator.indicator] = indicator
        ...
        if len(self.indicators) > self.max_indicators:
            ...
            for i in range(to_remove):
                ...
                if indicator.indicator in self.indicators_by_type[indicator.indicator_type]:
                    del self.indicators_by_type[indicator.indicator_type][indicator.indicator]
    ...
    def check_indicator(self, indicator: str, indicator_type: Optional[str] = None) -> Optional[ThreatIndicator]:
        """
    ...
    def get_indicators(self, indicator_type: Optional[str] = None,
                      source: Optional[str] = None, limit: int = 100) -> List[ThreatIndicator]:
        """
