...
class KafkaHandler(logging.Handler):
    """Handler that sends log messages to Kafka topic"""
    ...
    def __init__(self, bootstrap_servers, topic, **kwargs):
        ...
        try:
            ...
            print("Warning: kafka-python not installed. KafkaHandler disabled.")
            self.producer = None
    ...
    def emit(self, record):
        if self.producer:
            try:
                ...
                # Try to parse as JSON, if not, send as string
                try:
                    json_data = json.loads(data)
                except json.JSONDecodeError:
                    json_data = {"message": data, "timestamp": record.created}
...
class ValkeyHandler(logging.Handler):
    """Handler that sends log messages to Valkey/Redis"""
    
    def __init__(self, host, port, db=0, channel=None, password=None, **kwargs):
        super().__init__()
        try:
            import redis
            ...
            print("Warning: redis not installed. ValkeyHandler disabled.")
            self.client = None
        except Exception as e:
    ...
    def emit(self, record):
        if self.client:
            try:
                ...
                if self.channel:
                    self.client.publish(self.channel, json.dumps(json_data))
                
                # Store in Valkey for quick lookup (last 24 hours)
                event_id = f"opencanary:event:{record.created}:{record.name}"
                self.client.setex(event_id, 3600 * 24, json.dumps(json_data))
                
            except Exception as e:
                print(f"Error sending to Valkey: {e}")
