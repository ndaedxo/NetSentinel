...
from netsentinel.siem_integration import SiemManager, get_siem_manager
from netsentinel.sdn_integration import SDNManager, get_sdn_manager
...
def performance_timer():
    ...
    class Timer:
        ...
        def assert_under_threshold(self, threshold_seconds: float, operation: str = "operation"):
            """Assert that operation completed within threshold"""
            elapsed = self.elapsed
            assert elapsed < threshold_seconds, f"{operation} took {elapsed:.2f}s (threshold: {threshold_seconds}s)"
...
def memory_monitor():
    ...
    class MemoryMonitor:
        ...
        def assert_under_threshold(self, threshold_bytes: int, operation: str = "operation"):
            """Assert that memory usage is under threshold"""
            current = self.get_current_usage()
            assert current < threshold_bytes, f"{operation} used {current} bytes (threshold: {threshold_bytes} bytes)"
...
def cleanup_singletons():
    ...
    yield
    # Reset singletons
    global _firewall_manager_instance
    global _siem_manager_instance
    global _sdn_manager_instance

    try:
        from netsentinel.firewall_manager import _firewall_manager_instance
        _firewall_manager_instance = None
    except:
        pass

    try:
        from netsentinel.siem_integration import siem_manager
        siem_manager = None
    except:
        pass

    try:
        from netsentinel.sdn_integration import sdn_manager
        sdn_manager = None
    except:
        pass
...
def assert_event_processed_successfully(processor, event_data, expected_score_range=None):
