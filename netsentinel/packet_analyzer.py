#!/usr/bin/env python3
"""
Packet-level network analysis for OpenCanary
...
try:
    ...
    print("Warning: Scapy not available. Packet analysis disabled.")
...
class PacketAnalyzer:
    ...
    def __init__(self, interface: str = "any", max_flows: int = 10000,
                 flow_timeout: int = 300, enable_analysis: bool = True):
        """
        ...
        # Configuration
        self.suspicious_ports = {22, 23, 80, 443, 3306, 3389, 5900}  # Common honeypot ports
        self.scan_detection_threshold = 10  # Packets per minute threshold for port scanning
    ...
    def _capture_packets(self):
        ...
        try:
            ...
            sniff(iface=self.interface, prn=packet_callback, store=0, stop_filter=lambda x: not self.running)
    ...
    def _parse_packet(self, packet) -> Optional[PacketInfo]:
        ...
        try:
            ...
            # UDP layer
                ...
                packet_info.payload_size = len(udp_layer.payload) if udp_layer.payload else 0

            # ICMP
            elif packet.haslayer(ICMP):
                packet_info.protocol = "ICMP"
                packet_info.payload_size = len(packet[ICMP].payload) if packet[ICMP].payload else 0
    ...
    def _analyze_packet_realtime(self, packet_info: PacketInfo) -> Optional[Dict]:
        ...
        if packet_info.protocol == "TCP" and packet_info.flags and "SYN" in packet_info.flags:
            ...
            recent_packets = self._get_recent_packets_from_ip(packet_info.src_ip, 60)  # Last minute
            syn_packets = [p for p in recent_packets if p.protocol == "TCP" and p.flags and "SYN" in p.flags]
        ...
        if packet_info.dst_port in self.suspicious_ports:
            ...
            recent_connections = self._get_recent_packets_to_port(packet_info.dst_port, 300)  # Last 5 minutes
            connections_from_ip = len([p for p in recent_connections if p.src_ip == packet_info.src_ip])
    ...
    def _analyze_traffic_periodic(self) -> List[Dict]:
        ...
        for flow_key, flow in self.active_flows.items():
            ...
            if flow.dst_port in self.suspicious_ports and flow.duration() > 600:  # 10 minutes
                anomalies.append({
    ...
    def _cleanup_old_flows(self):
        ...
        # Clean up flow queue
        self.flow_queue = deque([k for k in self.flow_queue if k in self.active_flows], maxlen=self.max_flows)
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'bytes_per_second': self.stats['total_bytes'] / runtime if runtime > 0 else 0,
            'protocol_counts': dict(self.stats['protocol_counts']),
            'top_ports': sorted(self.stats['port_counts'].items(), key=lambda x: x[1], reverse=True)[:10],
            'top_ips': sorted(self.stats['ip_counts'].items(), key=lambda x: x[1], reverse=True)[:10],
