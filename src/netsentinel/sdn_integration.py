...
logger = create_logger("sdn_integration", level="INFO")
...
class OpenDaylightController:
    ...
    def __init__(self, controller: SDNController):
        ...
        self.session.auth = (
            controller.username or "admin",
            controller.password or "admin",
        )
    ...
    def add_flow(self, switch_id: str, flow: FlowRule) -> bool:
        ...
        try:
                ...
                "flow-node-inventory:flow": [
                    {
                        ...
                        "instructions": {
                            "instruction": [
                                {"order": 0, "apply-actions": {"action": flow.actions}}
                            ]
                        },
                    }
                ]
            ...
            url = (
                f"{self.controller.base_url}/restconf/config/"
                f"opendaylight-inventory:nodes/node/{switch_id}/"
                f"table/{flow.table_id}"
            )
            response = self.session.post(
                url, json=flow_data, timeout=self.controller.timeout
            )
            response.raise_for_status()
    ...
    def delete_flow(self, switch_id: str, flow_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete flow {flow_id} from switch {switch_id}: {e}"
            )
            return False
...
class ONOSController:
    ...
    def __init__(self, controller: SDNController):
        ...
        self.session = requests.Session()
        self.session.auth = (
            controller.username or "onos",
            controller.password or "rocks",
        )
        self.session.headers.update({"Content-Type": "application/json"})
    ...
    def add_flow(self, device_id: str, flow: Dict[str, Any]) -> bool:
        ...
        try:
            url = f"{self.controller.base_url}/onos/v1/flows/{device_id}"
            response = self.session.post(
                url, json=flow, timeout=self.controller.timeout
            )
            response.raise_for_status()
    ...
    def delete_flow(self, device_id: str, flow_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete flow {flow_id} from device {device_id}: {e}"
            )
            return False
...
class RyuController:
    ...
    def add_flow(self, switch_id: str, flow: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            flow_data = {"dpid": int(switch_id, 16), **flow}  # Convert hex to int
            response = self.session.post(
                url, json=flow_data, timeout=self.controller.timeout
            )
            response.raise_for_status()
...
class SDNManager:
    ...
    def add_controller(self, controller: SDNController):
        ...
        logger.info(
            f"Added SDN controller: {controller.name} ({controller.type.value})"
        )
        ...
        try:
            ...
            for flow in flows:
                if controller.add_flow(switch_id, flow):
                    success_count += 1
                    flow_id = f"quarantine_{switch_id}_{flow.cookie}"
            ...
            if success_count > 0:
                ...
                logger.info(
                    f"Quarantined IP {target_ip} on switch {switch_id} for {duration}s"
                )
                return True
            else:
    ...
    def release_quarantine(self, policy_name: str) -> bool:
        ...
        try:
            ...
            flows_to_remove = [
                ...
                if flow_data.get("policy") == policy_name
            ]

            success_count = 0
            for flow_id in flows_to_remove:
                flow_data = self.active_flows[flow_id]
                if controller.delete_flow(
                    flow_data["switch_id"], flow_id.split("_")[-1]
                ):
                    success_count += 1
            ...
            if success_count > 0:
                ...
                logger.error(
                    f"Failed to remove quarantine flows for policy {policy_name}"
                )
                return False

        except Exception as e:
            logger.error(
                f"Failed to release quarantine {policy_name}: {type(e).__name__}: {str(e)}"
            )
            return False
    ...
    def redirect_traffic(
        ...
        try:
            ...
            flow = FlowRule(
                switch_id=switch_id,
                priority=200,
                match={"ipv4_src": source_ip, "eth_type": "0x0800"},
                actions=[
                    {"output-action": {"output-node-connector": destination_port}}
                ],
                idle_timeout=duration,
                cookie=int(time.time() * 1000),  # Use milliseconds for uniqueness
            )

            if controller.add_flow(switch_id, flow):
                flow_id = f"redirect_{switch_id}_{flow.cookie}"
                self.active_flows[flow_id] = {
                    "type": "redirection",
                    "controller": controller_name,
                    "switch_id": switch_id,
                ...
                logger.info(
                    f"Redirected traffic from {source_ip} to port {destination_port}"
                )
                return True
            else:
    ...
    def _create_quarantine_flows(self, policy: QuarantinePolicy) -> List[FlowRule]:
        ...
        if policy.quarantine_vlan:
            ...
            flow = FlowRule(
                ...
                priority=300,
                match={"ipv4_src": policy.target_ip, "eth_type": "0x0800"},
                actions=[
    ...
    def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                expired_policies = [
                    policy_name
                    for policy_name, policy in self.quarantine_policies.items()
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_quarantines": len(
                [p for p in self.quarantine_policies.values() if p.active]
            ),
...
def quarantine_threat_ip(
    controller_name: str, switch_id: str, ip_address: str, duration: int = 3600
) -> bool:
