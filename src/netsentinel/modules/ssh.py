...
class HoneyPotSSHUserAuthServer(userauth.SSHUserAuthServer):
    ...
    def auth_publickey(self, packet):
        ...
        try:
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.debug("Key was {key}".format(key=key))
...
class HoneyPotTransport(transport.SSHServerTransport):
    ...
    def ssh_KEXINIT(self, packet):
        # Log remote SSH version instead of printing
        self.logger.debug('Remote SSH version: %s' % (self.otherVersionString,))
...
class HoneyPotSSHFactory(factory.SSHFactory):
    ...
    def getPrimes(self):
        ...
        _modulis = [
            "/etc/ssh/moduli",
            "/private/etc/moduli",
            resource_filename("netsentinel", "data/moduli"),
        ]
        _primes = None
...
class HoneyPotSSHSession(session.SSHSession):
    def request_env(self, data):
        # Log environment request instead of printing
        self.logger.debug('request_env: %s' % (repr(data)))
        pass
