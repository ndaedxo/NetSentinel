...
PROFILES = {
    "ms-isa": {
        ...
        "status_reason": (
            b"Proxy Authentication Required ( The ISA Server requires authorization to fulfill the request. "
            b"Access to the Web Proxy service is denied.  )"
        ),
    },
    "squid": {
...
class AlertProxyRequest(Request):
    ...
    def logAuth(self):
        ...
        if atype == "Basic":
            ...
            logger.debug(b64decode(token).decode("utf-8").split(":"))
            exit(1)
            logger.debug("something NTLM")
