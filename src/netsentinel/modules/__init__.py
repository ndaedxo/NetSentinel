...
from ..honeycred import buildHoneyCredHook

# Create a base protocol class instead of monkey patching
from twisted.internet import protocol


class CanaryProtocol(protocol.Protocol):
    """TCP protocols (ie. descendants of this class) gain a log method that can
    be called with just the event data, as transport data is added here"""
    ...
    def log(self, *args, **kwargs):
        ...
        raise AttributeError(
            f"Instance of {self.__class__.__name__} does not have 'factory' attribute "
            "or factory does not have a log function."
        )


# Instead of monkey patching, provide a factory function for protocols
def create_canary_protocol(base_protocol_class):
    """Create a protocol class that inherits from both base_protocol_class and CanaryProtocol"""
    class HybridProtocol(CanaryProtocol, base_protocol_class):
        pass
...
class CanaryService(object):
    ...
    def log(self, logdata, **kwargs):
        ...
        if logtype:
            msg = (
                "Passing in the logtype to log is deprecated. "
                "(In future each module will have only only logtype.)"
            )
