...
class TestSplunkConnector:
    ...
    @patch("netsentinel.siem_integration.requests.post")
    def test_batch_buffering(self, mock_post, splunk_connector):
        """Test event batching"""
        mock_post.return_value = Mock(status_code=200, text="Success")
        events = []
        for i in range(3):
...
class TestElkConnector:
    ...
    def elk_connector(self):
        ...
        return ElkConnector(
            elasticsearch_url="http://elasticsearch:9200", index_prefix="netsentinel"
        )
...
class TestSyslogConnector:
    ...
    def syslog_connector(self):
        ...
        return SyslogConnector(
            host="127.0.0.1", port=514, protocol="udp", hostname="test-host"
        )

    def test_send_udp_syslog(self, syslog_connector):
    ...
    def test_send_tcp_syslog(self, mock_socket_class):
        ...
        connector = SyslogConnector(host="127.0.0.1", port=514, protocol="tcp")
...
class TestWebhookSiemConnector:
    ...
    @patch("src.netsentinel.siem_integration.requests.Session.request")
    def test_send_webhook_event(self, mock_request, webhook_connector):
        ...
        # Headers may not be present in mock call
...
class TestSiemManager:
    ...
    def test_add_splunk_connector(self, siem_manager):
        ...
        siem_manager.add_splunk_connector(
            "test_splunk", "https://splunk.example.com:8088", "test-token"
        )
    ...
    def test_add_elk_connector(self, siem_manager):
        ...
        siem_manager.add_elk_connector("test_elk", "http://elasticsearch:9200")
    ...
    def test_send_event_processing(self, mock_convert, siem_manager):
        ...
        with patch.object(
            siem_manager.connectors["webhook_test_webhook"],
            "send_event",
            return_value=True,
        ):
    ...
    def test_buffer_flushing(self, mock_post, siem_manager):
        ...
        for i in range(3):
            siem_manager.send_event(
                {"logtype": "4002", "threat_score": 8.0, "src_host": f"192.168.1.{i}"}
            )
...
class TestSiemManagerIntegration:
    ...
    @patch("src.netsentinel.siem_integration.requests.Session.request")
    def test_performance_under_load(self, mock_request, siem_manager):
