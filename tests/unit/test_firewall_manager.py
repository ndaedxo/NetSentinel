...
class TestFirewallManager:
    ...
    def test_detect_firewall_type(self, firewall_manager):
        """Test firewall type detection"""
        # Mock subprocess to simulate different firewall systems
        with patch('subprocess.run') as mock_run:
            # Test iptables detection
            mock_run.return_value = Mock(returncode=0, stdout="iptables detected")
            firewall_type = firewall_manager._detect_firewall_type()
            assert firewall_type in ['iptables', 'ufw', 'firewalld', 'nftables']

            # Test ufw detection
            mock_run.side_effect = [
                Mock(returncode=1),  # iptables not found
                Mock(returncode=0, stdout="ufw detected")  # ufw found
            ]
            firewall_type = firewall_manager._detect_firewall_type()
            assert firewall_type == 'ufw'

    def test_ip_blocking_iptables(self, firewall_manager):
        """Test IP blocking with iptables"""
        test_ip = "192.168.1.100"

        with patch.object(firewall_manager, '_detect_firewall_type', return_value='iptables'), \
            ...
            mock_run.assert_called_with(
                ['iptables', '-I', 'INPUT', '-s', test_ip, '-j', 'DROP', '-m', 'comment', '--comment', 'NetSentinel: test block'],
    ...
    def test_ip_blocking_ufw(self, firewall_manager):
        ...
        with patch.object(firewall_manager, '_detect_firewall_type', return_value='ufw'), \
             patch('subprocess.run') as mock_run:
    ...
    def test_ip_blocking_firewalld(self, firewall_manager):
        ...
        with patch.object(firewall_manager, '_detect_firewall_type', return_value='firewalld'), \
             patch('subprocess.run') as mock_run:
    ...
    def test_ip_unblocking(self, firewall_manager):
        ...
        with patch.object(firewall_manager, '_detect_firewall_type', return_value='iptables'), \
            ...
            unblock_calls = [call for call in mock_run.call_args_list if '-D' in call[0][0]]
    ...
    def test_ip_status_check(self, firewall_manager):
        ...
        with patch.object(firewall_manager, '_detect_firewall_type', return_value='iptables'), \
            ...
            mock_run.return_value = Mock(returncode=0, stdout=f"-A INPUT -s {test_ip} -j DROP")

            result = firewall_manager.is_ip_blocked(test_ip)
            assert result is True

            # Mock iptables output showing the IP is not blocked
            mock_run.return_value = Mock(returncode=0, stdout="-A INPUT -s 10.0.0.1 -j DROP")
...
"""
    ...
    def test_ipv6_support(self, firewall_manager):
        ...
        with patch.object(firewall_manager, '_detect_firewall_type', return_value='iptables'), \
            ...
            ipv6_calls = [call for call in mock_run.call_args_list if 'ip6tables' in call[0][0]]
