...
from netsentinel.sdn_integration import (
    SDNManager, SDNController, SDNControllerType,
    FlowRule, QuarantinePolicy, get_sdn_manager,
    OpenDaylightController, ONOSController, RyuController
)
...
class TestSDNManager:
    ...
    @patch('netsentinel.sdn_integration.OpenDaylightController')
...
class TestSDNManagerIntegration:
    ...
    def test_full_sdn_workflow(self, sdn_manager):
        ...
        with patch('netsentinel.sdn_integration.OpenDaylightController') as mock_odl_class:
            mock_controller = Mock()
            mock_odl_class.return_value = mock_controller
            mock_controller.add_flow.return_value = True
            mock_controller.delete_flow.return_value = True
    ...
    def test_performance_under_load(self, sdn_manager):
        ...
        with patch.object(sdn_manager, 'get_controller_interface') as mock_get_interface:
            mock_controller = Mock()
            mock_controller.add_flow.return_value = True
    ...
    def test_security_validations(self, sdn_manager):
        ...
        result = sdn_manager.quarantine_ip("test_odl", malicious_switch, "192.168.1.100")
        ...
        result = sdn_manager.quarantine_ip("test_odl", "openflow:1", malicious_ip)
        assert result is False
